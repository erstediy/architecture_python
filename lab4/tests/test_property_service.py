...
AGENT = {"id": "aaaaaaaaaaaaaaaaaaaaaaaa", "login": "agent1", "first_name": "A", "last_name": "B", "role": "agent"}
BUYER = {"id": "bbbbbbbbbbbbbbbbbbbbbbbb", "login": "buyer1", "first_name": "C", "last_name": "D", "role": "buyer"}
...
def test_update_status_not_found(prop_client_with_override):
    override(AGENT)
    resp = prop_client_with_override.patch("/api/v1/properties/aaaaaaaaaaaaaaaaaaaaaaaa/status", json={"status": "sold"}, headers=AUTH_HEADER)
...
def test_create_viewing_not_found(prop_client_with_override):
    override(BUYER)
    resp = prop_client_with_override.post("/api/v1/viewings", json={"property_id": "aaaaaaaaaaaaaaaaaaaaaaaa", "scheduled_at": "2025-06-01T10:00:00"}, headers=AUTH_HEADER)
    assert resp.status_code == 404
